var jsAnimate = function () {
    ...
    function slide(object) {
        ...
        var node, intervalTime, slideTime
    ...
    function slick(object) {
        ...
        var node, slideTime, number, height, quick, row, imgScale
        node = document.getElementById(object.id)
        slideTime = object.speed || 0.3
        quick = object.quick
        row = object.row
    ...
    function fade() {
        ...
        for (var i = 0; i < lazyDocuments.length; i++) {
            ...
            if (lazyDocuments[i].getAttribute('duration')) {
                ...
                //首次匹配
                var reg1 = /^\{\s*(\w+)\s*:\s*([^\s][^,\}]*)/i
                //二次及以后匹配
                var reg2 = /^,\s*(\w+)\s*:\s*([^\s][^,\}]*)/i
                //第一次
                if (reg1.test(durationString)) {
                    key = RegExp.$1
                    value = RegExp.$2.trim()
                    nodeData.duration[key] = value
                    //匹配完就可以删掉了(从vue源码里的AST部分获得的灵感)
            ...
            function enterTo(lazyNode) {
                        ...
                        if (object.duration.target && object.duration.target === 'target' + postfix) {
                            if (object.delay) {
                                ...
                                if (object.duration.animation) {
                                    var animationTime = (parseFloat(getComputedStyle(object.node,null).getPropertyValue('animation-duration')) || 0) + (parseFloat(getComputedStyle(object.node,null).getPropertyValue('animation-delay')) || 0)
                ...
                if (lazyNode.duration && lazyNode.duration.animation) {
                    ...
                    if (lazyNode.duration.type) {
                        ...
                        switch (type) {
                            case "transition":
                                //appendChild似乎不能立刻在页面中渲染,导致改变了样式是立刻出现的，而不是经过transition才出现，所以这里用异步函数，当整段函数运行结束再改变样式
                                setTimeout(function () {
